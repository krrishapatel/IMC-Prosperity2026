...
# ──────────────────────────────────────────────────────────────────────────────
# Strategy parameters  (tuned on R1 + R2 historical data)
# ──────────────────────────────────────────────────────────────────────────────
...
# ── Ash-Coated Osmium ─────────────────────────────────────────────────────────
# Mean-reverting near 10_000; symmetric MM + takes (unchanged vs v1).
...
# ── Intarian Pepper Root (v2: aggressive trend exploiter) ─────────────────
# Trend: +0.001/ts. Oscillation ~2-3 ticks. No sells.
#
# Phase 1: sweep every ask strictly below fair (cheap vs rising trend).
# Phase 2: maker bid at int(fair) - PEPPER_MAKER_OFFSET so oscillation hits
#          the quote much sooner than old min(best_bid+1, fair-1) (~fair-6).
PEPPER_TREND_PER_TS  = 0.001
PEPPER_MAKE_SIZE     = 16
PEPPER_MAKER_OFFSET  = 3
...
class Trader:
    ...
    def _pepper_orders(self, depth: OrderDepth, fair: float, position: int) -> list[Order]:
        """
        v2: max-long fast, then hold.

        Phase 1 — Take: all ask levels with price < fair (vs +0.001/ts trend).
        Phase 2 — Maker: bid at int(fair) - PEPPER_MAKER_OFFSET so ~2-3 tick noise
                 crosses the quote. No sells.
        """
        orders: list[Order] = []
        ...
        bid_q = int(fair) - PEPPER_MAKER_OFFSET
