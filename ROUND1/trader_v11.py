...
# Stable, tested configuration (high score and low crash risk)
OSM_BUY_LEVEL = 9998
...
PEPPER_START_TS = 3000
PEPPER_EARLY_EDGE = 2.0
PEPPER_EARLY_CHUNK = 10
PEPPER_WIDE_SPREAD_SKIP = 12
...
class Trader:
    ...
    def _book_mid(self, depth: OrderDepth):
    ...
    def run(self, state: TradingState):
        ...
        if osm_depth is not None and osm_depth.buy_orders and osm_depth.sell_orders:
        ...
        # Pepper fast accumulation + tactical harvest
        pepper_depth = state.order_depths.get("INTARIAN_PEPPER_ROOT")
        if pepper_depth is not None and pepper_depth.sell_orders:
            ...
            orders: list[Order] = []
            best_bid = max(pepper_depth.buy_orders.keys()) if pepper_depth.buy_orders else None
            best_ask = min(pepper_depth.sell_orders.keys())
            spread = (best_ask - best_bid) if best_bid is not None else 0

            if (
                state.timestamp >= PEPPER_START_TS
                and spread <= PEPPER_WIDE_SPREAD_SKIP
                and buy_cap > 0
                and pos < PEPPER_TARGET_POS
            ):
                if state.timestamp <= PEPPER_BOOTSTRAP_TS:
                    edge = PEPPER_EARLY_EDGE
                    chunk = PEPPER_EARLY_CHUNK
                else:
                    edge = PEPPER_NORMAL_EDGE
                    chunk = PEPPER_ORDER_CHUNK
                for ask in sorted(pepper_depth.sell_orders.keys()):
                    if buy_cap <= 0 or pos >= PEPPER_TARGET_POS:
                        break
                    if ask > fair + edge:
                        break
                    qty = min(-pepper_depth.sell_orders[ask], buy_cap, PEPPER_TARGET_POS - pos, chunk)
                    if qty > 0:
                        orders.append(Order("INTARIAN_PEPPER_ROOT", ask, qty))
                        buy_cap -= qty
                        pos += qty

            if pos > PEPPER_HARVEST_FLOOR and pepper_depth.buy_orders:
