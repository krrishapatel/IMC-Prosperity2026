...
OSM_BUY_LEVEL = 9_992
OSM_SELL_LEVEL = 10_008
OSM_ORDER_CHUNK = 12

# v3 idea: get long as quickly as possible, then mostly hold.
PEPPER_TARGET_POS = 80
PEPPER_BOOTSTRAP_TS = 2_000
PEPPER_BOOTSTRAP_EDGE = 14.0
PEPPER_NORMAL_EDGE = 5.0
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # Osmium: strict take-only mean reversion, no passive quoting.
        ...
        if osm_depth is not None:
            ...
            logger.print(f"ASH_COATED_OSMIUM pos={state.position.get('ASH_COATED_OSMIUM', 0)} orders={len(osm_orders)}")
        ...
        if pepper_depth is not None:
            ...
            logger.print(
                f"INTARIAN_PEPPER_ROOT pos={state.position.get('INTARIAN_PEPPER_ROOT', 0)} "
                f"fair={fair:.1f} buys={len(orders)}"
            )
