...
class Trader:
    ...
    def __init__(self):
        ...
        self.squid_ink_midprices = []

        # hyperparameters
        self.squid_window_length = 1000
        self.low_z = -3
        self.high_z = 3
        self.use_z_scores = True
    ...
    def trade_squid_ink(self, state):
        ...
        if len(sell_orders) != 0 and len(buy_orders) != 0:
            ...
            # Window logic
            self.squid_ink_midprices.append(decimal_fair_price)
            self.squid_ink_midprices = self.squid_ink_midprices[-self.squid_window_length:] # keep only the last window length midprices
            
            z_score = None

            if len(self.squid_ink_midprices) == self.squid_window_length:
                # Calculate z-scores
                mean = np.mean(self.squid_ink_midprices)
                std_dev = np.std(self.squid_ink_midprices)
                z_score = (decimal_fair_price - mean) / std_dev
            

            if self.use_z_scores:
                # if squid ink z-score below a threshold, expect market to mean revert, so BUY more!
                if z_score is not None and z_score < self.low_z:
                    decimal_fair_price += 1 # we expect fair price is higher than it is 

                elif z_score is not None and z_score > self.high_z:
                    decimal_fair_price -= 1 # we expect fair price is lwoer than it is
            ...
            if best_ask is not None and best_bid is not None:
                ...
                # if squid ink z-score above a threshold, expect market to mean revert, so SELL more!
