...
class Trader:
    ...
    def trade_squid(self, state):
        ...
        # --- Get mid price ---
        if len(order_book.buy_orders) == 0 or len(order_book.sell_orders) == 0:
            return
        ...
        long_sma = np.mean(self.squid_ink_long_window_prices)

        # --- Init trade state ---
        if not hasattr(self, 'squid_trade_state'):
            self.squid_trade_state = {
                'active': False,
            ...
            for trade in market_trades:
                if trade.quantity >= 15:
                    ...
                    if mid_price < short_sma:
                        ...
                        qty = 15
                        if qty > 0:
                            self.send_buy_order(product, best_ask, qty, msg="SQUID: ENTER LONG")
                            self.squid_trade_state = {'active': True, 'side': 'long'}
                            break
                    elif mid_price > short_sma:
                        # Sell signal
                        qty = 15
                        if qty > 0:
                            self.send_sell_order(product, best_bid, -qty, msg="SQUID: ENTER SHORT")
