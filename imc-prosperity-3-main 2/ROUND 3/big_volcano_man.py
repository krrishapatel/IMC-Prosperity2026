...
class Trader:
   ...
   # =======================================
    ...
    def initialize_round_3(self):
        ...
        self.days_left = 6 # for every additional round, decrease this buy 1, day 0 : 8, day 1 : 7, day 2: 6, day 3 (submission) : 5
        ...
        self.spreads = {
            'VOLCANIC_ROCK_VOUCHER_9500' :  0.75,
            'VOLCANIC_ROCK_VOUCHER_9750' :  0.75,
            'VOLCANIC_ROCK_VOUCHER_10000' : 0.75,
            'VOLCANIC_ROCK_VOUCHER_10250' : 0.75,
            'VOLCANIC_ROCK_VOUCHER_10500' : 0.75
        }
    ...
    def mm_on_IV(self, state): 
        for voucher in self.vouchers:
            ...
            logger.print(f"{strike} Market  {bid} | {ask} -> Eating Orders: {sent_buys} | {sent_sells}")
            ...
            # logger.print(f"Raw Delta: {self.total_raw_delta_positive:.2f} | -{self.total_raw_delta_negative:.2f}")
            if bid == ask:
    ...
    def trade_underlying(self, state, size):         
        if size < 0:
            ...
            self.send_buy_order('VOLCANIC_ROCK', price, size, msg=f"Buying Coupons {size} @ {price}")
            ...
            self.send_buy_order('VOLCANIC_ROCK', price, -size, msg=f"Selling Coupons {size} @ {price}")
    ...
    def delta_hedge(self, state):
        ...
        for voucher in self.vouchers:
            ...
            # delta = np.mean(self.voucher_deltas[voucher]) # take average delta
    ...
    def trade_vouchers(self, state):
        ...
        # first hedge our delta
        # logger.print("== hedging delta from last step ==")
        self.delta_hedge(state)
        # logger.print("== trade iv model ==")
        self.mm_on_IV(state)
