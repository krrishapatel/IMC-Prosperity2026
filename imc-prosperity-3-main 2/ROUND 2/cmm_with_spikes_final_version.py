...
class Trader:
    ...
    def __init__(self):
        ...
        self.prev_avg_vol = None

        self.volatility_averages = []

        # squid hyperparams
        self.volatility_window = 50     

        # windows
        self.squid_ink_long_window = 340   # long window size for squid ink
        self.squid_ink_short_window = 200   # short window size for squid ink
            ...
            for ask, amount in orders[0:max(len(orders), depth)]: 
                ...
                if int(ask) < acceptable_price or (abs(ask - acceptable_price) < 1 and (pos < 0 and abs(pos - amount) < abs(pos))):
                    if product == 'RAINFOREST_RESIN':
                        ...
                        # only need to check this if we are short since we are looking for BUYS
    ...
    def get_worst_bid(self, state, product):
        order_depth = state.order_depths[product]
        if len(order_depth.buy_orders) != 0:
            orders = list(order_depth.buy_orders.items())
            bid, _ = orders[-1]
    ...
    def get_worst_ask(self, state, product):
        order_depth = state.order_depths[product]
        if len(order_depth.sell_orders) != 0:
            orders = list(order_depth.sell_orders.items())
            ask, _ = orders[-1]
    ...
    def momentum_trading(self, state, mid_price, spread, position, best_bid, best_ask):
    ...
    def trade_squid(self, state):
        ...
        if len(sell_orders) != 0 and len(buy_orders) != 0:
            ...
            if len(self.squid_ink_prices) < self.squid_ink_long_window:
                ...
                spike_trading = False
                if len(self.volatility_window_price_diffs) > 1:
                    # price diff
                    price_diff = abs(self.volatility_window_price_diffs[-1])
